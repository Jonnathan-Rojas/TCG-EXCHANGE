...
// INICIO BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DE ALTA DE PRODUCTO
...
$pdo = Bd::getPdo();
// FIN BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DE ALTA DE PRODUCTO


// INICIO BLOQUE: ACCESO POR POST CON id_binder (SIN GET CON DATOS)
...
if (tcgx_client_binders_obtener($pdo, $tcgxIdBinder, $idUsuarioVista) === null) {
    $_SESSION['tcgx_binders_flash'] = ['tipo' => 'error', 'texto' => 'EL BINDER NO EXISTE O NO LE PERTENECE.'];
    header('Location: binders.php', true, 303);
    exit;
}
// FIN BLOQUE: ACCESO POR POST CON id_binder


// INICIO BLOQUE: PROCESAMIENTO POST DE ALTA CON IMAGENES
$tcgxFormValores = ['cantidad' => 1, 'tipomoneda' => 'COLONES', 'publicado' => 0];
$tcgxFormErrores = [];
...
if ($esGuardar) {
    ...
    if ($errorRevalidacion !== null) {
        ...
        $tcgxFormValores = array_merge($tcgxFormValores, $_POST);
    } else {
        $validacion = tcgx_client_productos_validar($_POST);
        $tcgxFormErrores = $validacion['errores'];
        $tcgxFormValores = array_merge($tcgxFormValores, $validacion['datos']);

        $imagenesValidadas = [];
        if (isset($_FILES['imagenes']) && is_array($_FILES['imagenes'])) {
            $imagenesValidadas = tcgx_client_productos_validar_imagenes($_FILES['imagenes'], $tcgxFormErrores);
        }

        if (empty($tcgxFormErrores)) {
            $resultado = tcgx_client_productos_crear($pdo, $tcgxIdBinder, $validacion['datos'], $idUsuarioVista, $idUsuarioVista);
            if ($resultado['ok']) {
                $idProducto = (int) $resultado['id'];
                if ($imagenesValidadas !== []) {
                    $resImg = tcgx_client_productos_guardar_imagenes(
                        $pdo,
                        $idProducto,
                        $tcgxIdBinder,
                        $validacion['datos']['nombrecarta'],
                        $imagenesValidadas,
                        $idUsuarioVista,
                        $idUsuarioVista
                    );
                    if (!$resImg['ok']) {
                        tcgx_client_productos_eliminar($pdo, $idProducto, $idUsuarioVista, $idUsuarioVista);
                        $tcgxFormErrores[] = $resImg['error'];
                    }
                }
                if (empty($tcgxFormErrores)) {
                    $_SESSION['tcgx_binders_flash'] = ['tipo' => 'ok', 'texto' => 'PRODUCTO CREADO CORRECTAMENTE.'];
...
// FIN BLOQUE: PROCESAMIENTO POST DE ALTA CON IMAGENES

$tcgxPageTitle = 'Crear producto | TCG EXCHANGE';
$tcgxFormModo = 'crear';
      ...
      <div class="tcgx-client-main">
         ...
         <main class="tcgx-client-content" id="tcgx-client-main">
            ...
            <div class="tcgx-client-form-card">
               <form method="post" action="producto-crear.php" id="tcgx-producto-form" enctype="multipart/form-data" novalidate>
                  ...
                  <?php
                  $tcgxFormValores['id_binder'] = $tcgxIdBinder;
                  require __DIR__ . '/includes/producto_formulario.php';
                  ?>
