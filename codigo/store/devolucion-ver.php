...
// INICIO BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DEL MODULO DEVOLUCIONES (STORE)
require __DIR__ . '/includes/carga_sesion_store.php';
require __DIR__ . '/includes/store_devoluciones_logica.php';
...
$pdo = Bd::getPdo();
// FIN BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DEL MODULO DEVOLUCIONES (STORE)


// INICIO BLOQUE: ACCESO POR POST Y CARGA DEL ENVIO ACOTADO A TIENDA
...
$tcgxDev = $idEnvio === '' ? null : tcgx_store_devoluciones_obtener($pdo, $idEnvio, $idTiendaSesion);
if ($tcgxDev === null) {
    $_SESSION['tcgx_devoluciones_flash'] = ['tipo' => 'error', 'texto' => 'EL ENVIO INDICADO NO EXISTE, NO ESTA EN DEVOLUCION O NO PERTENECE A SU TIENDA.'];
    header('Location: devoluciones.php', true, 303);
    exit;
}
...
// FIN BLOQUE: ACCESO POR POST Y CARGA DEL ENVIO ACOTADO A TIENDA


// INICIO BLOQUE: PROCESAMIENTO DE AVANCE CON REVALIDACION DE PERTENENCIA Y PERMISO DE TIENDA
$tcgxFormErrores = [];
if (isset($_POST['tcgx_dev_avanzar'])) {
    if (!tcgx_store_devoluciones_puede_avanzar_tienda($tcgxDev, $idTiendaSesion)) {
        $tcgxFormErrores[] = 'SU TIENDA NO ES RESPONSABLE DEL SIGUIENTE PASO EN ESTA DEVOLUCION.';
    } else {
        $validacion = tcgx_devoluciones_validar_avance($tcgxDev, $_POST);
        $tcgxFormErrores = $validacion['errores'];

        if (empty($tcgxFormErrores)) {
            $envioRevalidado = tcgx_store_devoluciones_obtener($pdo, $idEnvio, $idTiendaSesion);
            if ($envioRevalidado === null || !tcgx_store_devoluciones_puede_avanzar_tienda($envioRevalidado, $idTiendaSesion)) {
                $tcgxFormErrores[] = 'EL ESTADO DEL ENVIO CAMBIO; NO ES POSIBLE AVANZAR.';
            } else {
                $resultado = tcgx_devoluciones_avanzar($pdo, $envioRevalidado, $validacion['datos']['detalle'], $idUsuarioVista);
                if ($resultado['ok']) {
...
// FIN BLOQUE: PROCESAMIENTO DE AVANCE CON REVALIDACION DE PERTENENCIA Y PERMISO DE TIENDA
...
$puedeAvanzar = tcgx_store_devoluciones_puede_avanzar_tienda($tcgxDev, $idTiendaSesion);
      ...
      <div class="tcgx-store-main">
         ...
         <main class="tcgx-store-content" id="tcgx-store-main">
            ...
            <?php if ($puedeAvanzar && $siguienteEstado !== null): ?>
               <!-- INICIO BLOQUE: AVANZAR ESTADO DE DEVOLUCION (SOLO SI LA TIENDA ES RESPONSABLE) -->
