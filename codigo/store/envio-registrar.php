...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($tcgxEnviosCsrf === '' || $tokenPost === '' || !hash_equals($tcgxEnviosCsrf, $tokenPost)) {
        ...
        $validacion = tcgx_store_envios_validar_registro(Bd::getPdo(), $_POST, $idTiendaSesion);
        ...
        if (empty($tcgxFormErrores)) {
            ...
            if (empty($tcgxFormErrores)) {
                // Revalidacion operativa en BD antes de la mutacion sensible (diseño.md).
                $errorRevalidacion = tcgx_store_revalidar_operacion(Bd::getPdo(), $idUsuarioVista, $idTiendaSesion);
...
$tcgxTiendasDestino = tcgx_store_envios_listar_tiendas_destino(Bd::getPdo(), $idTiendaSesion);
