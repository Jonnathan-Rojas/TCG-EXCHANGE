...
$tcgxCons = $idConsolidado === '' ? null : tcgx_store_consolidados_obtener($pdo, $idConsolidado, $idTiendaSesion);
if ($tcgxCons === null) {
    $_SESSION['tcgx_consolidados_flash'] = ['tipo' => 'error', 'texto' => 'EL CONSOLIDADO INDICADO NO EXISTE O NO PERTENECE A SU TIENDA.'];
    header('Location: consolidados.php', true, 303);
    exit;
}
...
if (isset($_POST['tcgx_cons_accion'])) {
    ...
    if ($accion === 'despachar' && tcgx_store_consolidados_puede_despachar($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_despachar($pdo, $tcgxCons, (string) ($_POST['guiaexterna'] ?? ''), $idUsuarioVista);
    } elseif ($accion === 'recibir' && tcgx_store_consolidados_puede_recibir($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_recibir($pdo, $tcgxCons, (array) ($_POST['recepcion'] ?? []), $idUsuarioVista);
    } elseif ($accion === 'cancelar' && tcgx_store_consolidados_puede_cancelar($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_cancelar($pdo, $tcgxCons, $idUsuarioVista);
    } elseif ($accion === 'sacar' && tcgx_store_consolidados_puede_sacar($tcgxCons, $idTiendaSesion)) {
...
$puedeDespachar = tcgx_store_consolidados_puede_despachar($tcgxCons, $idTiendaSesion);
$puedeRecibir = tcgx_store_consolidados_puede_recibir($tcgxCons, $idTiendaSesion);
$puedeCancelar = tcgx_store_consolidados_puede_cancelar($tcgxCons, $idTiendaSesion);
$puedeSacar = tcgx_store_consolidados_puede_sacar($tcgxCons, $idTiendaSesion);
