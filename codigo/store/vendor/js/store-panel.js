/**
 * Panel tienda (store): cierre del offcanvas al usar anclas del menu movil y colapsar sidebar en escritorio.
 */
(function () {
	'use strict';
	 ...
	 */
	var offcanvasEl = document.getElementById('tcgx-store-offcanvas-nav');
	if (offcanvasEl && typeof window.bootstrap !== 'undefined') {
		var offcanvas = window.bootstrap.Offcanvas.getOrCreateInstance(offcanvasEl);
		offcanvasEl.querySelectorAll('.tcgx-store-sidebar__nav a.nav-link').forEach(function (a) {
	 ...
	 */
	var LS_SIDEBAR = 'tcgexchange_store_sidebar_collapsed';
	var rootApp = document.getElementById('tcgx-store-app-root');
	var btnToggle = document.getElementById('tcgx-store-btn-toggle-sidebar');

	function tcgexchangeStoreSyncSidebarButton() {
		if (!btnToggle || !rootApp) {
			return;
		}
		var collapsed = rootApp.classList.contains('tcgx-store-app--sidebar-collapsed');
	...
	if (rootApp && window.localStorage.getItem(LS_SIDEBAR) === '1') {
		rootApp.classList.add('tcgx-store-app--sidebar-collapsed');
	}
	tcgexchangeStoreSyncSidebarButton();

	if (btnToggle && rootApp) {
		btnToggle.addEventListener('click', function () {
			rootApp.classList.toggle('tcgx-store-app--sidebar-collapsed');
			var on = rootApp.classList.contains('tcgx-store-app--sidebar-collapsed');
			try {
