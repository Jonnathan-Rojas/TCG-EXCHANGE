...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($tcgxTarifasCsrf === '' || $tokenPost === '' || !hash_equals($tcgxTarifasCsrf, $tokenPost)) {
        ...
        $validacion = tcgx_tarifas_validar(Bd::getPdo(), $_POST);
        $tcgxFormErrores = $validacion['errores'];
        $tcgxFormValores = $validacion['datos'];
        if (empty($tcgxFormErrores)) {
            $resultado = tcgx_tarifas_crear(Bd::getPdo(), $validacion['datos'], $idUsuarioVista);
            if ($resultado['ok']) {
                $_SESSION['tcgx_tarifas_flash'] = ['tipo' => 'ok', 'texto' => 'TARIFA CREADA CORRECTAMENTE.'];
...
$tcgxTiendasOpciones = tcgx_tarifas_listar_tiendas(Bd::getPdo());
$tcgxRutasOpciones = tcgx_tarifas_listar_rutas(Bd::getPdo());
// FIN BLOQUE: CATALOGOS PARA EL FORMULARIO (TIENDAS Y RUTAS ACTIVAS)

$tcgxPageTitle = 'Crear tarifa | TCG EXCHANGE';
$tcgxFormModo = 'crear';
?>
<!DOCTYPE html>
<html lang="es">
      ...
      <div class="tcgx-admin-main">
         ...
         <main class="tcgx-admin-content" id="tcgx-admin-main">
            ...
            <div class="tcgx-admin-form-card">
               <form method="post" action="tarifa-crear.php" id="tcgx-tarifa-form" novalidate>
