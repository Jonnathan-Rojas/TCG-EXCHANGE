...
/**
 * Formulario de REGISTRO DE ENVIO INDIVIDUAL (admin). Se incluye dentro del <form> de envio-registrar.php.
 * Campos: forma de envio (ruta), tiendas de origen/destino (no hub), centro de distribucion (informativo,
 * unico y automatico), monto (automatico, solo lectura), remitente y destinatario (Select2 por nombre) y
 * filas dinamicas de paquetes. Repuebla los valores tras un reintento fallido. Salida escapada (anti-XSS).
 *
 * Variables esperadas del archivo que lo incluye:
 *   $tcgxTiendas (array id/nombre, no hub), $tcgxHubUnico (array|null), $tcgxRutas (array id/nombre),
 *   $tcgxFormValores (array), $tcgxPaquetesPrev (array de filas crudas para repoblar).
 */

$esc = static fn ($v): string => htmlspecialchars((string) $v, ENT_QUOTES, 'UTF-8');

// Lecturas seguras de valores previos.
$vForma = (string) ($tcgxFormValores['formaenvio'] ?? '');
$vOrigen = (string) ($tcgxFormValores['idtiendaorigen'] ?? '');
...
// Filas de paquetes para repoblar: las crudas del reintento o una fila vacia en alta limpia.
...
<!-- INICIO BLOQUE: DATOS GENERALES DEL ENVIO -->
<div class="row g-3">
   <div class="col-12 col-md-3">
      <label class="form-label" for="envio-forma">Forma de envío</label>
      <select class="form-select" id="envio-forma" name="formaenvio" required>
   ...
   <div class="col-12 col-md-3">
      ...
      <select class="form-select" id="envio-origen" name="idtiendaorigen" required>
         <option value="">SELECCIONE…</option>
         <?php foreach ($tcgxTiendas as $t): ?>
            <?php $tid = (string) $t['id']; ?>
            <option value="<?php echo $esc($tid); ?>" <?php echo $tid === $vOrigen ? 'selected' : ''; ?>><?php echo $esc($t['nombre']); ?></option>
         <?php endforeach; ?>
      </select>
   </div>
   ...
   <div class="col-12 col-md-3 d-none" id="envio-hub-wrap">
      ...
      <!-- HUB unico, asignado automaticamente por el servidor. Solo lectura (sin name: no se envia). -->
   ...
   <div class="col-12 col-md-3">
      ...
      <!-- Monto de solo lectura: lo calcula el servidor desde la tarifa de la tienda de origen para la forma de envio. -->
...
<!-- INICIO BLOQUE: PERSONAS (REMITENTE / DESTINATARIO) - BUSQUEDA DINAMICA POR NOMBRE (SELECT2) -->
...
<div id="tcgx-paquetes-contenedor">
   ...
   <?php foreach ($paquetesPrev as $pq): ?>
      ...
      <div class="row g-2 align-items-end tcgx-paquete-fila mb-2">
         ...
         <!-- INICIO BLOQUE: IMAGENES DE EVIDENCIA DEL PAQUETE (OPCIONAL, SOLO AL REGISTRAR) -->
         <div class="col-12">
            <label class="form-label">Imágenes (evidencia, opcional)</label>
            <!-- El name indexa por fila (paquete_imagenes[INDICE][]); el JS lo renumera al agregar/quitar. -->
         ...
         <!-- FIN BLOQUE: IMAGENES DE EVIDENCIA DEL PAQUETE -->
      </div>
      <?php $indicePaquete++; ?>
   <?php endforeach; ?>
</div>
<!-- FIN BLOQUE: PAQUETES (FILAS DINAMICAS) -->
