/**
 * Listado de rutas (catalogo) del modulo admin: inicializa DataTables (es, responsive),
...
(function () {
	...
	if (window.jQuery && jQuery.fn && jQuery.fn.DataTable) {
		jQuery('#tcgx-tabla-rutas').DataTable({
			responsive: true,
			pageLength: 10,
	 ...
	 * Editar: POST directo a ruta-editar.php. Estado: confirmacion SweetAlert2 antes de enviar.
	 */
	...
	function manejarAccion(boton) {
		...
		if (accion === 'estado') {
			...
			Swal.fire({
				icon: 'warning',
				title: esDesactivar ? '¿Desactivar ruta?' : '¿Activar ruta?',
				text: (esDesactivar ? 'Se desactivará (estado INACTIVO) la ruta ' : 'Se activará (estado ACTIVO) la ruta ') + nombre + '.',
	...
	var contenedorTabla = document.getElementById('tcgx-tabla-rutas');
	if (contenedorTabla) {
	...
	function mostrarFlash() {
		var nodo = document.getElementById('tcgx-rutas-flash');
