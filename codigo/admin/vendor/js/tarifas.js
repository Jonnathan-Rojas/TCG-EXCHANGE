/**
 * Listado de tarifas del modulo admin: inicializa DataTables (es, responsive),
...
(function () {
	...
	if (window.jQuery && jQuery.fn && jQuery.fn.DataTable) {
		jQuery('#tcgx-tabla-tarifas').DataTable({
			responsive: true,
			pageLength: 10,
	 ...
	 * Editar: POST directo a tarifa-editar.php. Eliminar: confirmacion SweetAlert2 antes de enviar.
	 */
	...
	function manejarAccion(boton) {
		...
		if (accion === 'eliminar') {
			...
			var tramo = boton.getAttribute('data-tcgx-tramo') || '';
			...
			Swal.fire({
				...
				text: 'Se eliminará la tarifa ' + tramo + ' de ' + nombre + '. Esta acción no se puede deshacer.',
	...
	var contenedorTabla = document.getElementById('tcgx-tabla-tarifas');
	if (contenedorTabla) {
	...
	function mostrarFlash() {
		var nodo = document.getElementById('tcgx-tarifas-flash');
