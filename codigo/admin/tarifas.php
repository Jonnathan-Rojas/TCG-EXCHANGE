<?php
declare(strict_types=1);

// INICIO BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DEL CRUD DE TARIFAS
require __DIR__ . '/includes/carga_sesion_admin.php';
require __DIR__ . '/includes/tarifas_logica.php';

// Token CSRF propio del CRUD de tarifas: independiente del token rotado por carga_sesion_admin,
// para que los formularios de listado y de alta/edicion validen sin colisionar con la regeneracion del bootstrap.
if (empty($_SESSION['tcgx_tarifas_csrf'])) {
    $_SESSION['tcgx_tarifas_csrf'] = bin2hex(random_bytes(32));
}
$tcgxTarifasCsrf = $_SESSION['tcgx_tarifas_csrf'];
// FIN BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DEL CRUD DE TARIFAS


// INICIO BLOQUE: LECTURA DE MENSAJE FLASH (PRG)
// Consume en un solo uso el resultado de alta/edicion/eliminacion para mostrarlo via SweetAlert2 tras la redireccion.
$tcgxTarifasFlash = null;
if (isset($_SESSION['tcgx_tarifas_flash']) && is_array($_SESSION['tcgx_tarifas_flash'])) {
    $tcgxTarifasFlash = $_SESSION['tcgx_tarifas_flash'];
    unset($_SESSION['tcgx_tarifas_flash']);
}
// FIN BLOQUE: LECTURA DE MENSAJE FLASH (PRG)


// INICIO BLOQUE: PROCESAMIENTO POST DE ELIMINACION
// Borrado fisico (dato de configuracion sin estado) validado con el token propio del CRUD; patron PRG hacia el listado.
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['tcgx_tarifas_eliminar'])) {
    $tokenPost = (string) ($_POST['tcgx_csrf_token'] ?? '');
    $idRaw = trim((string) ($_POST['id_tarifa'] ?? ''));

    if ($tcgxTarifasCsrf === '' || $tokenPost === '' || !hash_equals($tcgxTarifasCsrf, $tokenPost)) {
        $_SESSION['tcgx_tarifas_flash'] = ['tipo' => 'error', 'texto' => 'SOLICITUD NO VALIDA.'];
    } elseif ($idRaw === '' || !ctype_digit($idRaw)) {
        $_SESSION['tcgx_tarifas_flash'] = ['tipo' => 'error', 'texto' => 'TARIFA NO INDICADA.'];
    } else {
        $resultadoEliminar = tcgx_tarifas_eliminar(Bd::getPdo(), (int) $idRaw, $idUsuarioVista);
        if ($resultadoEliminar['ok']) {
            $_SESSION['tcgx_tarifas_flash'] = ['tipo' => 'ok', 'texto' => 'TARIFA ELIMINADA CORRECTAMENTE.'];
        } else {
            $_SESSION['tcgx_tarifas_flash'] = ['tipo' => 'error', 'texto' => $resultadoEliminar['error']];
        }
    }

    header('Location: ' . $tcgxAdminScriptNombre, true, 303);
    exit;
}
// FIN BLOQUE: PROCESAMIENTO POST DE ELIMINACION


// INICIO BLOQUE: CONSULTA DE LISTADO
$tcgxListaTarifas = tcgx_tarifas_listar(Bd::getPdo());
// FIN BLOQUE: CONSULTA DE LISTADO

$tcgxPageTitle = 'Tarifas | TCG EXCHANGE';
?>
<!DOCTYPE html>
<html lang="es">

<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title><?php echo htmlspecialchars($tcgxPageTitle, ENT_QUOTES, 'UTF-8'); ?></title>
   <meta name="robots" content="noindex, nofollow">

   <link rel="icon" href="<?php echo htmlspecialchars($tcgxAdminUrlFavicon, ENT_QUOTES, 'UTF-8'); ?>" type="image/png" sizes="512x512">

   <link rel="preconnect" href="https://fonts.googleapis.com">
   <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
   <link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&family=Rethink+Sans:ital,wght@0,400..800;1,400..800&display=swap" rel="stylesheet">

   <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css" integrity="sha512-DTOQO9RWCH3ppGqcWaEA1BIZOC6xxalwEsw9c2QQeAIftl+Vegovlnee1c9QX4TctnWMn13TZye+giMm8e2LwA==" crossorigin="anonymous" referrerpolicy="no-referrer">
   <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/sweetalert2@11.14.5/dist/sweetalert2.min.css" crossorigin="anonymous">

   <!-- INICIO BLOQUE: ESTILOS DATATABLES (BOOTSTRAP 5 + RESPONSIVE)
        Tablas con buscador, selector de filas y paginador; en movil detalle desplegable (diseño.md). -->
   <link rel="stylesheet" href="https://cdn.datatables.net/2.1.8/css/dataTables.bootstrap5.min.css" crossorigin="anonymous">
   <link rel="stylesheet" href="https://cdn.datatables.net/responsive/3.0.3/css/responsive.bootstrap5.min.css" crossorigin="anonymous">
   <!-- FIN BLOQUE: ESTILOS DATATABLES (BOOTSTRAP 5 + RESPONSIVE) -->

   <link rel="stylesheet" href="vendor/css/admin-panel.css?v=20260612c">
</head>

<body class="tcgx-admin-app" id="tcgx-admin-app-root">

   <div class="tcgx-admin-app__body">

      <?php require __DIR__ . '/includes/sidebar.php'; ?>

      <div class="tcgx-admin-main">

         <?php require __DIR__ . '/includes/header.php'; ?>

         <main class="tcgx-admin-content" id="tcgx-admin-main">

            <!-- INICIO BLOQUE: BARRA DE ACCION DE ALTA SOBRE LA TABLA
                 Boton de alta alineado a la derecha en su propia fila, encima de la tabla. -->
            <div class="tcgx-admin-tabla-toolbar">
               <a class="btn btn-success" href="tarifa-crear.php">
                  <i class="fa-solid fa-plus me-2" aria-hidden="true"></i>Crear tarifa
               </a>
            </div>
            <!-- FIN BLOQUE: BARRA DE ACCION DE ALTA SOBRE LA TABLA -->

            <!-- INICIO BLOQUE: TABLA DE TARIFAS (DATATABLES) -->
            <div class="tcgx-admin-table-card">
               <table class="table table-hover align-middle tcgx-admin-dt-table" id="tcgx-tabla-tarifas">
                  <thead>
                     <tr>
                        <th>ID</th>
                        <th>Tienda</th>
                        <th>Ruta</th>
                        <th>Precio</th>
                        <th>Registro</th>
                        <th class="text-end">Acciones</th>
                     </tr>
                  </thead>
                  <tbody>
                     <?php foreach ($tcgxListaTarifas as $tarifa): ?>
                        <?php
                        // Variables escapadas por celda: salida segura en HTML (anti-XSS).
                        $tId = (int) $tarifa['id'];
                        $tIdEsc = htmlspecialchars((string) $tId, ENT_QUOTES, 'UTF-8');
                        $tNombreTienda = htmlspecialchars((string) ($tarifa['nombretienda'] ?? ''), ENT_QUOTES, 'UTF-8');
                        $tRuta = htmlspecialchars((string) ($tarifa['nombreruta'] ?? ''), ENT_QUOTES, 'UTF-8');
                        // PRECIO UNICO: se muestra precioporpaquete. NUNCA USAR PRECIO BASE (preciobase ignorado).
                        $tPrecio = htmlspecialchars(number_format((float) $tarifa['precioporpaquete'], 2, '.', ','), ENT_QUOTES, 'UTF-8');
                        $tFecha = htmlspecialchars((string) $tarifa['fecharegistro'], ENT_QUOTES, 'UTF-8');
                        ?>
                        <tr>
                           <td><?php echo $tIdEsc; ?></td>
                           <td><?php echo $tNombreTienda; ?></td>
                           <td><span class="tcgx-estado tcgx-estado--activo"><?php echo $tRuta; ?></span></td>
                           <td><?php echo $tPrecio; ?></td>
                           <td><?php echo $tFecha; ?></td>
                           <td class="text-end">
                              <div class="tcgx-admin-actions justify-content-end">
                                 <!-- Editar (naranja/warning): navega a la pagina de edicion por POST (sin GET con datos). -->
                                 <button type="button" class="btn btn-warning" data-tcgx-action="editar" data-tcgx-id="<?php echo $tIdEsc; ?>" title="Editar" aria-label="Editar tarifa <?php echo $tIdEsc; ?>">
                                    <i class="fa-solid fa-pen" aria-hidden="true"></i>
                                 </button>
                                 <!-- Eliminar (rojo/danger): borrado fisico con confirmacion (dato de configuracion sin estado). -->
                                 <button type="button" class="btn btn-danger" data-tcgx-action="eliminar" data-tcgx-id="<?php echo $tIdEsc; ?>" data-tcgx-nombre="<?php echo $tNombreTienda; ?>" data-tcgx-tramo="<?php echo $tRuta; ?>" title="Eliminar" aria-label="Eliminar tarifa <?php echo $tIdEsc; ?>">
                                    <i class="fa-solid fa-trash" aria-hidden="true"></i>
                                 </button>
                              </div>
                           </td>
                        </tr>
                     <?php endforeach; ?>
                  </tbody>
               </table>
            </div>
            <!-- FIN BLOQUE: TABLA DE TARIFAS (DATATABLES) -->

         </main>

         <?php require __DIR__ . '/includes/footer.php'; ?>

      </div>
   </div>

   <?php
   $tcgxAdminSidebarModo = 'offcanvas';
   require __DIR__ . '/includes/sidebar.php';
   unset($tcgxAdminSidebarModo);
   ?>

   <!-- INICIO BLOQUE: FORMULARIOS OCULTOS PARA ACCIONES POST (EDITAR / ELIMINAR)
        Un solo formulario por accion, completado y enviado por JS; mantiene POST + CSRF sin repetir formularios por fila. -->
   <form id="tcgx-form-editar" method="post" action="tarifa-editar.php" class="d-none">
      <input type="hidden" name="tcgx_csrf_token" value="<?php echo htmlspecialchars($tcgxTarifasCsrf, ENT_QUOTES, 'UTF-8'); ?>">
      <input type="hidden" name="id_tarifa" id="tcgx-form-editar-id" value="">
   </form>
   <form id="tcgx-form-eliminar" method="post" action="<?php echo htmlspecialchars($tcgxAdminScriptNombre, ENT_QUOTES, 'UTF-8'); ?>" class="d-none">
      <input type="hidden" name="tcgx_tarifas_eliminar" value="1">
      <input type="hidden" name="tcgx_csrf_token" value="<?php echo htmlspecialchars($tcgxTarifasCsrf, ENT_QUOTES, 'UTF-8'); ?>">
      <input type="hidden" name="id_tarifa" id="tcgx-form-eliminar-id" value="">
   </form>
   <!-- FIN BLOQUE: FORMULARIOS OCULTOS PARA ACCIONES POST (EDITAR / ELIMINAR) -->

   <!-- INICIO BLOQUE: CARGA UTIL DEL FLASH PARA SWEETALERT2
        Datos del resultado de la operacion previa, escapados como JSON para consumo del JS del modulo. -->
   <?php if ($tcgxTarifasFlash !== null): ?>
      <script id="tcgx-tarifas-flash" type="application/json"><?php
         echo json_encode($tcgxTarifasFlash, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE);
      ?></script>
   <?php endif; ?>
   <!-- FIN BLOQUE: CARGA UTIL DEL FLASH PARA SWEETALERT2 -->

   <script src="https://code.jquery.com/jquery-3.7.1.min.js" integrity="sha256-/JqT3SQfAwRwa/8RTZXw0qPkhUjKtH+7kb7l857zCX9Rk=" crossorigin="anonymous"></script>
   <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
   <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.14.5/dist/sweetalert2.all.min.js" crossorigin="anonymous"></script>

   <!-- INICIO BLOQUE: SCRIPTS DATATABLES (BOOTSTRAP 5 + RESPONSIVE) -->
   <script src="https://cdn.datatables.net/2.1.8/js/dataTables.min.js" crossorigin="anonymous"></script>
   <script src="https://cdn.datatables.net/2.1.8/js/dataTables.bootstrap5.min.js" crossorigin="anonymous"></script>
   <script src="https://cdn.datatables.net/responsive/3.0.3/js/dataTables.responsive.min.js" crossorigin="anonymous"></script>
   <script src="https://cdn.datatables.net/responsive/3.0.3/js/responsive.bootstrap5.min.js" crossorigin="anonymous"></script>
   <!-- FIN BLOQUE: SCRIPTS DATATABLES (BOOTSTRAP 5 + RESPONSIVE) -->

   <script src="vendor/js/admin-panel.js?v=20260612a"></script>
   <script src="vendor/js/tarifas.js?v=20260612a"></script>
</body>
</html>
