...
// Estado por defecto del formulario; en error se conservan los valores enviados.
$tcgxFormValores = ['estado' => 'ACTIVO'];
$tcgxFormErrores = [];
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($tcgxUsuariosCsrf === '' || $tokenPost === '' || !hash_equals($tcgxUsuariosCsrf, $tokenPost)) {
        ...
        $validacion = tcgx_usuarios_validar(Bd::getPdo(), $_POST, false);
        $tcgxFormErrores = $validacion['errores'];
        $tcgxFormValores = $validacion['datos'];
        if (empty($tcgxFormErrores)) {
            $resultado = tcgx_usuarios_crear(Bd::getPdo(), $validacion['datos'], $idUsuarioVista);
            if ($resultado['ok']) {
                // Entrega unica de la clave generada via flash; PRG al listado.
                $_SESSION['tcgx_usuarios_flash'] = [
                    'tipo' => 'clave_generada',
                    'modo' => 'creada',
                    'id' => $validacion['datos']['id'],
...
$tcgxFormTiendas = tcgx_usuarios_listar_tiendas(Bd::getPdo());
$tcgxCatalogoGeo = tcgx_usuarios_catalogo_geografico();
// FIN BLOQUE: CATALOGOS PARA EL FORMULARIO

$tcgxPageTitle = 'Crear usuario | TCG EXCHANGE';
$tcgxFormModo = 'crear';
?>
<!DOCTYPE html>
<html lang="es">
      ...
      <div class="tcgx-admin-main">
         ...
         <main class="tcgx-admin-content" id="tcgx-admin-main">
            ...
            <!-- INICIO BLOQUE: TARJETA FORMULARIO DE ALTA
                 La contraseña se genera automaticamente al guardar y se muestra una sola vez en el listado. -->
            <div class="tcgx-admin-form-card">
               <form method="post" action="usuario-crear.php" id="tcgx-usuario-form" novalidate>
