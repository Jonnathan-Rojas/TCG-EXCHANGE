...
// INICIO BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DE MI PERFIL
// La pagina opera siempre sobre el usuario en sesion; el identificador jamas viaja por la URL ni por POST.
...
$pdo = Bd::getPdo();
// FIN BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DE MI PERFIL


// INICIO BLOQUE: LECTURA DE FLASH DE EXITO (PRG)
// Tras guardar sin cambio de clave se redirige aqui (GET) y se consume en un solo uso el mensaje de exito.
...
if ($usuarioActual === null) {
    // Caso defensivo: sesion valida pero sin fila; se fuerza salida limpia.
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['tcgx_perfil_guardar'])) {
    ...
    if ($tcgxUsuariosCsrf === '' || $tokenPost === '' || !hash_equals($tcgxUsuariosCsrf, $tokenPost)) {
        ...
        // Conserva los datos reintroducidos para repintar el formulario ante error.
        ...
        if (empty($tcgxFormErrores)) {
            ...
            if ($resultado['ok']) {
                if (!empty($resultado['clave_cambiada'])) {
                    // INICIO BLOQUE: CIERRE DE SESION TRAS CAMBIO DE CONTRASENA (regla de seguridad de diseño.md)
                    // Cambio de credencial exitoso: invalidar la sesion y exigir nuevo inicio con la nueva contrasena.
                    $_SESSION = [];
                ...
                // Sin cambio de clave: PRG al propio perfil con mensaje de exito.
...
<body class="tcgx-admin-app" id="tcgx-admin-app-root">
      ...
      <div class="tcgx-admin-main">
         ...
         <main class="tcgx-admin-content" id="tcgx-admin-main">
            ...
            <!-- INICIO BLOQUE: TARJETA FORMULARIO MI PERFIL
                 Datos sensibles (cédula, nombre, perfil) en solo lectura; el resto editable y contraseña manual opcional. -->
            <div class="tcgx-admin-form-card">
               <form method="post" action="mi-perfil.php" id="tcgx-perfil-form" novalidate>
                  ...
                  <div class="row g-3">
                     ...
                     <!-- Linea 1 (solo lectura): Cédula, Nombre, Perfil -->
                     ...
                     <!-- Linea 2 (editable): Correo, Teléfono, Provincia -->
                     <div class="col-12 col-md-4">
                     ...
                     <!-- Linea 3 (editable): Cantón, Distrito, Dirección -->
                     <div class="col-12 col-md-4">
                  ...
                  <!-- INICIO BLOQUE: SECCION CAMBIO DE CONTRASENA (opcional; en blanco mantiene la actual) -->
                  <h3 class="tcgx-admin-form-subtitle">Cambiar contraseña</h3>
                  <div class="row g-3">
                     <div class="col-12 col-md-4">
   ...
   <?php elseif ($tcgxPerfilExito !== null): ?>
      ...
      ?></script>
   <?php endif; ?>
   <!-- FIN BLOQUE: DATOS PARA JS (CATALOGO GEOGRAFICO, ERRORES Y EXITO DE SERVIDOR) -->
