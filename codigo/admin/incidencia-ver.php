...
// INICIO BLOQUE: ACCESO POR POST (SIN GET CON DATOS) Y CARGA DE LA INCIDENCIA
...
$tcgxInc = $idIncidencia > 0 ? tcgx_incidencias_obtener(Bd::getPdo(), $idIncidencia) : null;
if ($tcgxInc === null) {
    $_SESSION['tcgx_incidencias_flash'] = ['tipo' => 'error', 'texto' => 'LA INCIDENCIA INDICADA NO EXISTE.'];
    header('Location: incidencias.php', true, 303);
    exit;
}
...
if (!isset($_POST['tcgx_inc_actualizar'])) {
    tcgx_auditorias_registrar_lectura(Bd::getPdo(), $idUsuarioVista, 'incidencias', (string) $idIncidencia);
}
// FIN BLOQUE: AUDITORIA DE LECTURA DEL DETALLE
// FIN BLOQUE: ACCESO POR POST Y CARGA DE LA INCIDENCIA


// INICIO BLOQUE: PROCESAMIENTO DE ACTUALIZACION (CAMBIO DE ESTADO + HISTORIAL)
$tcgxFormErrores = [];
if (isset($_POST['tcgx_inc_actualizar'])) {
    ...
    if (empty($tcgxFormErrores)) {
        $resultado = tcgx_incidencias_actualizar(Bd::getPdo(), $tcgxInc, $validacion['datos'], $idUsuarioVista);
        if ($resultado['ok']) {
...
$tcgxHistorial = tcgx_incidencias_historial(Bd::getPdo(), $idIncidencia);
      ...
      <div class="tcgx-admin-main">
         ...
         <main class="tcgx-admin-content" id="tcgx-admin-main">
            ...
            <div class="tcgx-admin-table-card mb-3">
               ...
               <?php else: ?>
                  <table class="table table-hover align-middle tcgx-admin-dt-table" id="tcgx-tabla-historial">
                     <thead>
                        <tr>
                           <th>Fecha</th>
                           <th>Estado</th>
