...
/**
 * Arranque comun de paginas del modulo cd: BD, sesion, logout POST, autorizacion TIENDA (eshub=1),
 * CSRF, datos de cabecera y URLs de imagenes. Incluir una sola vez al inicio de cada PHP bajo cd/.
 */
...
$tcgxCdScriptNombre = basename((string) ($_SERVER['SCRIPT_NAME'] ?? 'index.php'));

// INICIO BLOQUE: CIERRE SESION CENTRO DE DISTRIBUCION POST
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['tcgx_cd_logout'])) {
    $tokenSesion = $_SESSION['tcgx_cd_csrf'] ?? '';
    ...
    header('Location: ' . $tcgxCdScriptNombre, true, 303);
    exit;
}
// FIN BLOQUE: CIERRE SESION CENTRO DE DISTRIBUCION POST

// INICIO BLOQUE: AUTORIZACION PERFIL TIENDA HUB (eshub=1)
...
try {
    ...
    $tcgxCdEshub = (int) ($filaAuth['eshub'] ?? 0);
    if ($tcgxCdEshub !== 1) {
        header('Location: ../store/', true, 302);
        exit;
    }
    $tcgxCdNombreUsuario = trim((string) $filaAuth['nombreusuario']);
    $tcgxCdNombreTienda = trim((string) $filaAuth['nombretienda']);
} catch (Throwable) {
    header('Location: ../login.php', true, 302);
    exit;
}

if ($tcgxCdNombreUsuario === '') {
    $tcgxCdNombreUsuario = $idUsuarioVista;
}

$_SESSION['tcgx_cd_csrf'] = bin2hex(random_bytes(32));
$tcgxCdCsrf = $_SESSION['tcgx_cd_csrf'];
// FIN BLOQUE: AUTORIZACION PERFIL TIENDA HUB (eshub=1)

// INICIO BLOQUE: URL RECURSOS ESTATICOS (images/)
$tcgxCdUrlLogoOscuro = tcgexchange_url_recurso_proyecto('images/logo-on-dark.svg');
$tcgxCdUrlFavicon = tcgexchange_url_recurso_proyecto('images/logo512.png');
