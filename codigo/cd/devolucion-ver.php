...
$tcgxDev = $idEnvio === '' ? null : tcgx_cd_devoluciones_obtener($pdo, $idEnvio, $idTiendaSesion);
if ($tcgxDev === null) {
    $_SESSION['tcgx_devoluciones_flash'] = ['tipo' => 'error', 'texto' => 'EL ENVIO INDICADO NO EXISTE, NO ESTA EN DEVOLUCION O NO TRANSITA POR ESTE CENTRO.'];
    header('Location: devoluciones.php', true, 303);
    exit;
}
...
// INICIO BLOQUE: PROCESAMIENTO DE AVANCE CON REVALIDACION Y PERMISO DEL HUB
$tcgxFormErrores = [];
if (isset($_POST['tcgx_dev_avanzar'])) {
    ...
    } elseif (!tcgx_cd_devoluciones_puede_avanzar_hub($tcgxDev, $idTiendaSesion)) {
        $tcgxFormErrores[] = 'ESTE CENTRO NO ES RESPONSABLE DEL SIGUIENTE PASO EN ESTA DEVOLUCION.';
    } else {
        $validacion = tcgx_devoluciones_validar_avance($tcgxDev, $_POST);
        $tcgxFormErrores = $validacion['errores'];

        if (empty($tcgxFormErrores)) {
            $envioRevalidado = tcgx_cd_devoluciones_obtener($pdo, $idEnvio, $idTiendaSesion);
            if ($envioRevalidado === null || !tcgx_cd_devoluciones_puede_avanzar_hub($envioRevalidado, $idTiendaSesion)) {
...
// FIN BLOQUE: PROCESAMIENTO DE AVANCE CON REVALIDACION Y PERMISO DEL HUB
...
$puedeAvanzar = tcgx_cd_devoluciones_puede_avanzar_hub($tcgxDev, $idTiendaSesion);
