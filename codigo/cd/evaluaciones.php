...
// INICIO BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DEL CRUD DE EVALUACIONES (CD)
...
$pdo = Bd::getPdo();
// FIN BLOQUE: ARRANQUE, LOGICA Y TOKEN CSRF DEL CRUD DE EVALUACIONES (CD)
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['tcgx_evaluaciones_eliminar'])) {
    ...
    if ($tcgxEvaluacionesCsrf === '' || $tokenPost === '' || !hash_equals($tcgxEvaluacionesCsrf, $tokenPost)) {
        ...
        if ($errorRevalidacion !== null) {
            $_SESSION['tcgx_evaluaciones_flash'] = ['tipo' => 'error', 'texto' => $errorRevalidacion];
        } else {
            $idEvaluacion = (int) $idRaw;
            $evaluacionScope = tcgx_cd_evaluaciones_obtener($pdo, $idEvaluacion, $idTiendaSesion);
            if ($evaluacionScope === null) {
                $_SESSION['tcgx_evaluaciones_flash'] = ['tipo' => 'error', 'texto' => 'LA EVALUACION NO EXISTE O NO PERTENECE A ESTE CENTRO.'];
            } else {
...
$tcgxListaEvaluaciones = tcgx_cd_evaluaciones_listar($pdo, $idTiendaSesion);
...
<head>
   ...
   <link rel="icon" href="<?php echo htmlspecialchars($tcgxCdUrlFavicon, ENT_QUOTES, 'UTF-8'); ?>" type="image/png" sizes="512x512">
...
<body class="tcgx-cd-app" id="tcgx-cd-app-root">
      ...
      <div class="tcgx-cd-main">
         ...
         <main class="tcgx-cd-content" id="tcgx-cd-main">
            ...
            <div class="tcgx-cd-table-card">
               <table class="table table-hover align-middle tcgx-cd-dt-table" id="tcgx-tabla-evaluaciones">
                  <thead>
                     <tr>
                        <th>ID</th>
                        <th>Cliente</th>
   ...
   <form id="tcgx-form-eliminar" method="post" action="<?php echo htmlspecialchars($tcgxCdScriptNombre, ENT_QUOTES, 'UTF-8'); ?>" class="d-none">
