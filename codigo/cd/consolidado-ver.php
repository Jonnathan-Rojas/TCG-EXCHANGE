...
// INICIO BLOQUE: ACCESO POR POST (SIN GET CON DATOS) Y CARGA DEL CONSOLIDADO ACOTADO
...
$tcgxCons = $idConsolidado === '' ? null : tcgx_cd_consolidados_obtener($pdo, $idConsolidado, $idTiendaSesion);
if ($tcgxCons === null) {
    $_SESSION['tcgx_consolidados_flash'] = ['tipo' => 'error', 'texto' => 'EL CONSOLIDADO INDICADO NO EXISTE O NO PERTENECE A ESTE CENTRO.'];
    header('Location: consolidados.php', true, 303);
    exit;
}
...
if (!isset($_POST['tcgx_cons_accion'])) {
    tcgx_auditorias_registrar_lectura($pdo, $idUsuarioVista, 'consolidados', $idConsolidado);
}
// FIN BLOQUE: AUDITORIA DE LECTURA DEL DETALLE
// FIN BLOQUE: ACCESO POR POST Y CARGA DEL CONSOLIDADO


// INICIO BLOQUE: PROCESAMIENTO DE ACCIONES (REVALIDACION + PERMISOS STORE)
if (isset($_POST['tcgx_cons_accion'])) {
    $errorRevalidacion = tcgx_cd_revalidar_operacion($pdo, $idUsuarioVista, $idTiendaSesion);
    if ($errorRevalidacion !== null) {
        $_SESSION['tcgx_consolidados_flash'] = ['tipo' => 'error', 'texto' => $errorRevalidacion];
        header('Location: consolidados.php', true, 303);
        exit;
    }
    ...
    if ($accion === 'despachar' && tcgx_cd_consolidados_puede_despachar_tramo2($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_despachar($pdo, $tcgxCons, (string) ($_POST['guiaexterna'] ?? ''), $idUsuarioVista);
    } elseif ($accion === 'recibir' && tcgx_cd_consolidados_puede_recibir_tramo1($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_recibir($pdo, $tcgxCons, (array) ($_POST['recepcion'] ?? []), $idUsuarioVista);
    } elseif ($accion === 'cancelar' && tcgx_cd_consolidados_puede_cancelar_tramo2($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_cancelar($pdo, $tcgxCons, $idUsuarioVista);
    } elseif ($accion === 'sacar' && tcgx_cd_consolidados_puede_sacar_tramo2($tcgxCons, $idTiendaSesion)) {
        $resultado = tcgx_consolidados_sacar_envio($pdo, $tcgxCons, (string) ($_POST['idenvio'] ?? ''), $idUsuarioVista);
    } else {
...
// INICIO BLOQUE: CARGA DE DETALLE Y FRONTERAS DE ACCION STORE
$tcgxDetalle = tcgx_consolidados_detalle($pdo, $idConsolidado);
...
$puedeDespachar = tcgx_cd_consolidados_puede_despachar_tramo2($tcgxCons, $idTiendaSesion);
$puedeRecibir = tcgx_cd_consolidados_puede_recibir_tramo1($tcgxCons, $idTiendaSesion);
$puedeCancelar = tcgx_cd_consolidados_puede_cancelar_tramo2($tcgxCons, $idTiendaSesion);
$puedeSacar = tcgx_cd_consolidados_puede_sacar_tramo2($tcgxCons, $idTiendaSesion);
// FIN BLOQUE: CARGA DE DETALLE Y FRONTERAS DE ACCION STORE
   ...
   <div class="tcgx-cd-app__body">
      ...
      <div class="tcgx-cd-main">
         ...
         <main class="tcgx-cd-content" id="tcgx-cd-main">
            ...
            <!-- INICIO BLOQUE: ENVIOS INCLUIDOS (SACAR SOLO TRAMO 1 ORIGEN) -->
            ...
            <?php if ($puedeRecibir): ?>
               <!-- INICIO BLOQUE: RECEPCION PAQUETE POR PAQUETE (TRAMO 2 DESTINO) -->
               <div class="tcgx-cd-form-card mb-3">
            ...
            <!-- INICIO BLOQUE: ACCIONES DEL CONSOLIDADO (DESPACHAR/CANCELAR TRAMO 1) -->
